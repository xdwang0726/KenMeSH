...
def train(train_dataset, model, mlb, G, feats, edge_type, edge_norm, batch_sz, num_epochs, criterion,
    ...
    for epoch in range(num_epochs):
        for i, (label, text, title) in enumerate(train_data):
            label = torch.from_numpy(mlb.fit_transform(label)).type(torch.float)
            text, title, label = text.cuda(), title.cuda(), label.cuda()
            output = model(text, title, G, feats, edge_type, edge_norm)
...
def test(test_dataset, model, G, feats, edge_type, edge_norm, batch_sz):
    ...
    ori_label = []
    print('Testing....')
    for label, text, title in test_data:
...
def main():
    ...
    model = Multi_RGCN(vocab_size, args.nKernel, args.ksz, args.hidden_gcn_size, num_nodes, args.add_original_embedding,
                       args.atten_dropout, embedding_dim=args.embedding_dim, cornet_dim=1000, n_cornet_blocks=2)
