...
class dilatedCNN(nn.Module):
    def __init__(self, vocab_size, dropout, ksz, embedding_dim=200, rnn_num_layers=2):
        ...
        self.dconv = nn.Sequential(nn.Conv1d(self.embedding_dim*2, self.embedding_dim*2, kernel_size=self.ksz, padding=1, dilation=2),
                                   nn.SELU(), nn.AlphaDropout(p=0.05),
                                   nn.Conv1d(self.embedding_dim*2, self.embedding_dim*2, kernel_size=self.ksz, padding=1, dilation=5),
                                   nn.SELU(), nn.AlphaDropout(p=0.05),
                                   nn.Conv1d(self.embedding_dim*2, self.embedding_dim*2, kernel_size=self.ksz, padding=1, dilation=9),
                                   nn.SELU(), nn.AlphaDropout(p=0.05))
